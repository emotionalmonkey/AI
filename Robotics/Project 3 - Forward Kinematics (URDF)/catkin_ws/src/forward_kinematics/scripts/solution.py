    ...
    def compute_transforms(self, link_names, joints, joint_values):
        ...
        try:
            for i in range(len(link_names)):
                T  = numpy.dot(tf.transformations.translation_matrix(joints[i].origin.xyz), tf.transformations.quaternion_matrix(joints[i].origin.rpy))
                
                if joints[i].type == "revolute":
                    joint_index = joint_values.name.index(joints[i].name)
                    axis =   tf.transformations.quaternion_about_axis(joint_values.position[joint_index], joints[i].axis)
                    T = numpy.dot(T, tf.transformations.quaternion_matrix(axis))    
                    
                all_transforms.transforms.append(convert_to_message(T, link_names[i], "world_link" if i == 0 else link_names[i-1] ))
