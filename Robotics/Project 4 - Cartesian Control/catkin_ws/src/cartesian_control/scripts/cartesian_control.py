...
def cartesian_control(joint_transforms, b_T_ee_current, b_T_ee_desired,
    ...
    joint_transforms: a list containing the transforms of all the joints with respect to the base frame. 
                        In other words, joint_transforms[i] will contain the transform from 
                        the base coordinate frame to the coordinate frame of joint i.
    ...
    """    
    # transform from ee_current to ee_desired
    b_T_ee_changed = numpy.dot(numpy.linalg.inv(b_T_ee_current), b_T_ee_desired)

    # Translation is simply the last column
    changed_translation= b_T_ee_changed[:-1,-1] 

    # Rotation
    angle, axis = rotation_from_matrix(b_T_ee_changed)
    changed_rotation = angle * axis    

    # the desired change in end-effector pose from b_T_ee_current to b_T_ee_desired
    delta_x = numpy.array([changed_translation, changed_rotation])

    # propotional gain
    p_gain = 1
    x_dot = p_gain * delta_x

    # normalize the desired change to obtain a maximum end-effector velocity (e.g. 0.1 m/s and 1 rad/s)   
    t_norm = numpy.linalg.norm(x_dot[0])
    if  t_norm> 0.1:
        x_dot[0] *= 0.1/t_norm

    r_norm = numpy.linalg.norm(x_dot[1])
    if  r_norm> 1:
        x_dot[1] *= 1/r_norm

    V_ee = numpy.concatenate(x_dot,axis=0)
    
    # compute the robot Jacobian for each joint
    J = []
    for i in range(num_joints):
        joint = joint_transforms[i]

        joint_T = numpy.dot(numpy.linalg.inv(joint), b_T_ee_current)
        joint_translation = joint_T[:-1,-1]
        joint_rotation = joint_T[:-1,:-1].T

        # Skew matrix
        skew_matrix = S_matrix(joint_translation)        
       
        # (-1)rotation * skew(translation) = (-1) ee_R_j * S(j_t_ee)
        V_j = numpy.dot((-1) * joint_rotation, skew_matrix)        

        J.append(numpy.concatenate([V_j[:,-1], joint_rotation[:, -1]]))   
    
    J = numpy.transpose(J)
    
    # pseudo-inverse of the Jacobian
    J_plus = numpy.linalg.pinv(J, rcond=1e-2) # epsilon = 0.01 - Cutoff for small singular values. Singular values less than or equal to rcond * largest_singular_value are set to zero.

    dq = numpy.dot(J_plus, V_ee)

    """
    For null-space control:
    red_control: boolean telling you when the Cartesian Controller should take into account 
    the secondary objective for null-space control. 
    This is only set to True when the user interacts with the control marker dedicated to the first joint of the robot.
    q_current: list of all the current joint positions
    q0_desired: desired position of the first joint to be used as the secondary objective for null-space control. 
    Again, the goal of the secondary, null-space controller is to make the value of the first joint be 
    as close as possible to q0_desired, while not affecting the pose of the end-effector.

    The function must return a set of joint velocities such that the end-effector moves towards the desired pose. 
    If you are also implementing null-space control and red_control is set to true, 
    the joint velocities must also attempt to bring q[0] as close to q0_desired as possible, without affecting the primary goal above.
    If you choose to implement null-space control of the first joint, 
    find a joint velocity that also brings that joint closer to the secondary objective. 
    Use the Jacobian and its pseudo-inverse to project this velocity into the Jacobian nullspace. 
    Be careful to use the 'exact' version of the Jacobian pseudo-inverse, not its 'safe' version. 
    Then add the result to the joint velocities obtained for the primary objective
    """
    if red_control:
        # q_n = (identity matrix - J_plus * J) * q_dot 
        null_space = numpy.identity(num_joints) - numpy.dot(numpy.linalg.pinv(J), J) #'exact' version of J pseudo-inverse
        close_joint = numpy.zeros(num_joints)
        close_joint[0] = q0_desired - q_current[0]
        q_n = numpy.dot(null_space,close_joint)
        #rospy.loginfo("red_control Null space")
        #rospy.loginfo(q_n)
        dq = dq + q_n        
    #----------------------------------------------------------------------
    return dq
